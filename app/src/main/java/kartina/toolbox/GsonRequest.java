...
public class GsonRequest<T> extends Request<T> {
    ...
    public GsonRequest(int method, String url, Type typeOfSrc, String requestBody, Listener<T> listener,
                       ErrorListener errorListener) {
        super(method, url, errorListener);
        mGson = new Gson();
        mTypeOfSrc = typeOfSrc;
        mListener = listener;
        mRequestBody = requestBody;
    }
    ...
    protected Response<T> parseNetworkResponse(NetworkResponse response) {
        Log.e(getClass().getName(), RequestUtil.getInstance().toJsonString(response.data));
        try {
            String jsonString = new String(response.data, HttpHeaderParser.parseCharset(response.headers));
            T parsedGSON;
            if (mTypeOfSrc != null) {
                parsedGSON = mGson.fromJson(jsonString, mTypeOfSrc);
            } else {
